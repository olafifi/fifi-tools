...
- 右上角加入本地 24 小时数字时钟，以 `HH:MM:SS` 显示时间，并在下方显示日期和星期。
- 左下角拉链待办使用与布料开口完全同步的动态遮罩，拖到一半时只显示开口范围内的任务，收回时也不会穿帮。
- 五个小游戏统一为 FIFI Lab 的骨白纸张、黑色描边和硬阴影风格，同时分别保留莓红、玉绿、金色、砖红和深梅紫识别色。
- 点击工具卡片会先进入“蛋白传送门”过场页；新标签页地址栏立即显示真实的站内过渡地址，随后自动前往对应工具。
